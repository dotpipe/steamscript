...
try {
  ...
  // process.exit(1); // [DEBUG] Prevented shell exit (bcrypt not found)
  console.error('[DEBUG] Would exit: bcrypt not found');
}
...
function readLineRaw(prompt, hide, scriptedInputLines) {
  return new Promise(resolve => {
    ...
    function onData(char) {
      ...
      if (!escActive && char.charCodeAt(0) === 27) { // ESC
        escActive = true;
        escBuffer = char;
        return;
      }
      if (escActive) {
        escBuffer += char;
        // If buffer matches a known sequence, handle as normal
        if ([
          '\u001b[A','\u001b[B','\u001b[C','\u001b[D','\u001b[17;5~'
        ].includes(escBuffer)) {
          char = escBuffer;
          escActive = false;
          escBuffer = '';
        } else if (escBuffer.length > 1 && !/^[\u001b\[]/.test(char)) {
          // Unknown/garbled sequence, reset
          escActive = false;
          escBuffer = '';
      ...
      // Ctrl+C: exit
        ...
        stdin.pause();
        stdin.removeListener('data', onData);
        // process.exit(); // [DEBUG] Prevented shell exit (Ctrl+C)
        console.error('[DEBUG] Would exit: Ctrl+C pressed');
      }
      // F6 or Ctrl+F6: refresh shell (new process)
      else if (
        char === '\u001b[17;5~' || escBuffer === '^[[17;5~' || // Ctrl+F6
        char === '\u001b[17~'    || escBuffer === '^[[17~'     // F6
      ) {
        stdin.setRawMode(false);
        stdin.pause();
        stdin.removeListener('data', onData);
        ...
        spawn(process.argv[0], [shellPath], { stdio: 'inherit' });
        // process.exit(); // [DEBUG] Prevented shell exit (F6 refresh)
        console.error('[DEBUG] Would exit: F6 refresh');
      }
      // Backspace
      else if (char === '\u0008' || char === '\u007f') {
        if (cursor > 0) {
    ...
    if (shadow[username] && shadow[username].hash && bcrypt.compareSync(password, shadow[username].hash)) {
      ...
      if (failedAttempts >= 3) {
        ...
        if (challenge === hashPrefix) {
          ...
          // process.exit(1); // [DEBUG] Prevented shell exit (login failure)
          console.error('[DEBUG] Would exit: login failure');
        }
      }
      continue;
    }
...
function main() {
  ...
  // Add io.restorePrompt implementation
  ...
  (async () => {
    ...
    // ========== TERMINAL_OWNERSHIP_PROTOCOL ENFORCEMENT ========== //
    // See TERMINAL_OWNERSHIP_PROTOCOL.md for details.
    let terminalOwner = null; // null or string (app name)
    const stdin = process.stdin;

    // Protocol-compliant full-screen app launcher
    async function launchTerminalOwner(appName, fn) {
      if (terminalOwner) {
        process.stdout.write(`[ERROR] Terminal already owned by: ${terminalOwner}\n`);
        return;
      }
      terminalOwner = appName;
      try {
        // Block all shell input while app owns terminal
        stdin.setRawMode && stdin.setRawMode(false);
        stdin.pause && stdin.pause();
        await fn();
      } catch (e) {
        process.stdout.write(`[ERROR] Exception in ${appName}: ${(e && e.stack) ? e.stack : e}\n`);
      } finally {
        terminalOwner = null;
        // Restore shell input
    ...
    async function showDashboard() {
      await launchTerminalOwner('dashboard', async () => {
        try {
          let htodo;
          try {
          ...
          let result = await htodo([], { stdout: process.stdout, restorePrompt });
          while (result && result.goto === 'thread_viewer') {
            let threadViewer;
            try {
              threadViewer = require('./apps/thread_viewer');
            } catch (e) {
              process.stdout.write('[ERROR] Failed to require thread_viewer.js: ' + (e && e.stack ? e.stack : e) + '\n');
              break;
            }
            await launchTerminalOwner('thread_viewer', async () => {
              await threadViewer(result.threadId, { stdout: process.stdout }, '');
            });
            result = await htodo([], { stdout: process.stdout, restorePrompt });
          }
        } catch (e) {
          process.stdout.write('[htodo not available or error running dashboard]\n');
          process.stdout.write((e && e.stack ? e.stack : String(e)) + '\n');
        }
      });
    }

    // Only allow entering dashboard with F12, require 'q' to exit dashboard.
    function onDataWithF12(char) {
      if (terminalOwner) return; // Block input if terminal is owned
      // F12 key: '\u001b[24~' or '\u001b[21~' (varies by terminal)
      if (char === '\u001b[24~' || char === '\u001b[21~') {
        process.stdout.write('\x1b[2J\x1b[0;0H'); // Clear screen
        // Always resume shell after dashboard exits, regardless of return value
        showDashboard().then(() => {
          forcePrompt = true;
        }).catch(() => {
          forcePrompt = true;
    ...
    while (true) {
      try {
        ...
        do {
          if (terminalOwner) {
            // Block prompt/input while terminal is owned
        ...
        if (cmd === 'exit') break;
        // Detect thread_viewer launch and block prompt/input until it returns
        if (cmd === 'thread_viewer' || cmd === 'threadViewer') {
          await launchTerminalOwner('thread_viewer', async () => {
            try {
              const threadViewer = require('./apps/thread_viewer');
              await threadViewer(args[0], { stdout: process.stdout }, '');
            } catch (e) {
              process.stdout.write('[ERROR] Failed to run thread_viewer: ' + (e && e.stack ? e.stack : e) + '\n');
            }
          });
          continue;
        }
        ...
        process.stdout.write(`\n[SHELL LOOP ERROR]: ${err && err.stack ? err.stack : err}\n`);
        // Continue the loop no matter what
      }
    }

    // ========== END TERMINAL_OWNERSHIP_PROTOCOL ENFORCEMENT ========== //
  })();
}
